  ...
  const PAGE_FORM_FIELD_REMARKS = {
    driftRule: {
      ...
      applyType: s({
        ...
        readonly: '驱动子表必填性（物料/采样点 Tab 始终展示）',
      }),
      ...
      trApplyQcp: s({
        ...
        required: '否',
        format: '采样点编码；留空表示共用',
        readonly: '三类适用类型均展示；初始设置行可下拉；滚动行只读',
      }),
      trMainPeak: s({
        type: '单选',
        source: '手工录入',
    ...
    warningRule: {
      ...
      usl: s({
        ...
        required: '否',
        format: '比较符（≤/＜）+ 数值',
        readonly: '指标集 Tab 固定单行维护；列表默认展示',
      }),
      lsl: s({
        type: '阈值',
        source: '手工录入',
      ...
      ucl: s({
        ...
        readonly: '图谱判定超标新杂主阈值；指标集 Tab 维护；列表展示',
      }),
      lcl: s({
        type: '阈值',
        source: '手工录入',
      ...
      applyType: s({
        ...
        required: '是',
        format: '原材料 / 生产过程 / 生产物料',
        readonly: '无适用台账类型；三个 Tab 始终展示；驱动物料子表必填性',
      }),
      remark: s({ type: '文本', source: '手工录入', required: '否' }),
      enabled: s({
        type: '开关',
      ...
      matCode: s({
        ...
        required: '条件',
        unique: '子表内唯一',
        readonly: '适用物料 Tab；生产过程非必填；选择器仅展示启用物料',
      }),
      qcpCode: s({
        type: '引用',
        required: '是',
        unique: '子表内唯一',
        readonly: '采样点 Tab；不分适用类型均必填，至少一条',
      }),
    },
    recognition: {
      ledgerType: s({
