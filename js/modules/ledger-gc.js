...
(function () {
  ...
  const { SEED_LEDGER_GC } = ChromDriftSeedLedgerGc;
  ...
  window.ChromDriftLedgerGcPage = {
    name: 'LedgerGcPage',
    template: `
      <div class="page-module ledger-gc-page">
    ...
    setup() {
      ...
      const remarks = computed(() => ChromDriftRemarks.getPageRemarks('ledger-gc'));
      const flow = computed(() => ChromDriftRemarks.getPageFlow('ledger-gc'));
      ...
      const treeData = computed(() => {
        ...
        return ledger.value.categories
          .filter((c) => c.applyType === activeTab.value && c.ledgerType === 'GC')
      ...
      function persistAndRefresh() {
        ChromDriftStorage.save('ledger-gc-data', ledger.value);
        refreshMatrix();
      }
